...
namespace Book_Library
{
    public partial class Domains : Form
    {
        ...
        {
            OleDbCommand cmd1 = new OleDbCommand("SELECT COUNT(*) FROM Domenii WHERE denumire = '" + textBox2.Text + "' ");
            mycon.Open();
            ...
            if (denumireCount < 1)
            {
                ...
                cmd.CommandText = @"INSERT INTO Domenii (denumire) VALUES ('" + textBox2.Text + "')";
                cmd.Connection = mycon;
                cmd.ExecuteNonQuery();
            }
            else
            {
                MessageBox.Show("Domeniul introdus există deja!");
            }
            mycon.Close();
            this.domeniiTableAdapter.Fill(this._CNLR_2015_2016DataSet.Domenii);
        }
        ...
        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            ...
            if (count != 0)
            {
                OleDbCommand cmd1 = new OleDbCommand("DELETE FROM Domenii WHERE id = " + dataGridView1.SelectedRows[0].Cells[1].Value.ToString() + "");
                cmd1.Connection = mycon;
                mycon.Open();
                cmd1.ExecuteNonQuery();
                domeniiTableAdapter.Update(_CNLR_2015_2016DataSet.Domenii);
                mycon.Close();
                this.domeniiTableAdapter.Fill(this._CNLR_2015_2016DataSet.Domenii);
                if (sh == 1)
        ...
        private void textBox2_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                ...
                {
                    MessageBox.Show("Domeniul introdus exista deja!");
                }
                mycon.Close();
                this.domeniiTableAdapter.Fill(this._CNLR_2015_2016DataSet.Domenii);
            }
        ...
        private void toolStripTextBox1_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                ...
                OleDbCommand cmd = new OleDbCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = @"SELECT * FROM Domenii WHERE (denumire) LIKE ('%" + toolStripTextBox1.Text + "%')";
                mycon.Open();
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            if (count == 0)
            {
                dom = dataGridView1.SelectedCells[0].Value.ToString();
            }
            else
