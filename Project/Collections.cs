...
namespace Book_Library
{
    public partial class Collections : Form
    {
        ...
        {
            OleDbCommand cmd1 = new OleDbCommand("SELECT COUNT(*) FROM ColectiiCarti WHERE denumire = '" + textBox2.Text + "' ");
            mycon.Open();
            ...
            if (denumireCount < 1)
            {
                ...
                cmd.CommandText = @"INSERT INTO ColectiiCarti (denumire) VALUES ('" + textBox2.Text + "')";
                cmd.Connection = mycon;
                cmd.ExecuteNonQuery();
            }
            else
            {
                MessageBox.Show("Colecţia introdusă există deja!");
            }
            mycon.Close();
            this.colectiiCartiTableAdapter.Fill(this._CNLR_2015_2016DataSet.ColectiiCarti);
        }
        ...
        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            ...
            if (count != 0)
            {
                OleDbCommand cmd1 = new OleDbCommand("DELETE FROM ColectiiCarti WHERE id = " + dataGridView1.SelectedRows[0].Cells[1].Value.ToString() + "");
                cmd1.Connection = mycon;
                mycon.Open();
                cmd1.ExecuteNonQuery();
                colectiiCartiTableAdapter.Update(_CNLR_2015_2016DataSet.ColectiiCarti);
                mycon.Close();
                this.colectiiCartiTableAdapter.Fill(this._CNLR_2015_2016DataSet.ColectiiCarti);
                if (sh == 1)
        ...
        private void textBox2_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                ...
                {
                    MessageBox.Show("Colecţia introdusa există deja!");
                }
                mycon.Close();
                this.colectiiCartiTableAdapter.Fill(this._CNLR_2015_2016DataSet.ColectiiCarti);
            }
        ...
        private void toolStripTextBox1_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                ...
                OleDbCommand cmd = new OleDbCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = @"SELECT * FROM ColectiiCarti WHERE (denumire) LIKE ('%" + toolStripTextBox1.Text + "%')";
                mycon.Open();
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            if (count == 0)
            {
                col = dataGridView1.SelectedCells[0].Value.ToString();
            }
            else
